...
if [[ $EUID -ne 0 ]]; then
	...
	# Maia prompt
	PROMPT="%(!.%{$fg_bold[cyan]%}.)$USER@%{$fg_bold[cyan]%}%M %B%{$fg[cyan]%}%(4~|%-1~/.../%2~|%~)%u%b >%{$fg[cyan]%}>%B%(?.%{$fg[cyan]%}.%{$fg[cyan]%})>%{$reset_color%}%b $ " # Print some system information when the shell is first started
